...
describe("importCohortRoster", () => {
  ...
  describe("archived-cohort roster guard (TPO-2 closure audit)", () => {
    it("returns { archived: true } for an archived cohort, before touching the file at all", async () => {
      Cohort.findOne.mockReturnValueOnce(leanResult({ ...cohort, status: "archived" }));
    ...
    it("proceeds normally for a non-archived (active) cohort", async () => {
      Cohort.findOne.mockReturnValueOnce(leanResult({ ...cohort, status: "active" }));
      upsertCohortMembership.mockResolvedValue({ created: true, membership: { membershipStatus: "invited" } });
