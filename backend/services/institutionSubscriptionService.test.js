...
describe("institutionSubscriptionService", () => {
  ...
  it("treats active subscriptions without expiry as active", () => {
    expect(isInstitutionSubscriptionActive({
  ...
  it("keeps cancelled subscriptions active until their paid expiry", () => {
    ...
    expect(isInstitutionSubscriptionActive({ subscription }, new Date("2030-01-01T00:00:00.000Z"))).toBe(true);
    expect(isInstitutionSubscriptionActive({ subscription }, future)).toBe(false);
  });

  it("returns a normalized inactive state without a subscription", () => {
  ...
  it("rejects invalid plans before touching the database", async () => {
    await expect(
  ...
  it("writes a validated institution subscription", async () => {
    const saved = {
      _id: "college-1",
      subscription: { plan: "college_yearly", status: "active" },
    };
  ...
  it("cancels without removing expiry", async () => {
    ...
    await cancelInstitutionSubscription(
      "507f1f77bcf86cd799439011",
